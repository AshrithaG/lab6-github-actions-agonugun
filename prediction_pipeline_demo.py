...
def test_feature_columns_and_types(feature_target_sample):
    ...
    # numeric features + aligned lengths
    assert len(X) == len(y)
...
def test_main_block_runs_and_prints_score(capsys):
    """
    Execute the script via runpy so coverage counts the __main__ block,
    which reads Housing.csv and prints the score.
    """
